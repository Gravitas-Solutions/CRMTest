...
class Member extends Auth_Controller
{
    ...
    {        
        ...
        $this->data['saving'] = $toll_transactions * 0.185; 
        ...
        $this->data['daily_labels'] = ''; $this->data['daily_tolls'] = '';
        for ($i=1; $i <= $days; $i++) { 
        ...
        $this->data['agency_labels'] = ''; $this->data['agency_tolls'] = ''; $this->data['agency_colors'] = ''; 
        ...
        $this->data['monthly_labels'] = ''; $this->data['monthly_tolls'] = ''; 
        for ($i=1; $i <= 12; $i++) {
    ...
    function date_range_transactions()
    {
        ...
        $this->data['dept_name'] = ($dept == 0) ? 'All departments' : ucfirst($this->db->select('dept_name')->where('dept_id', $dept)->get('departments')->row()->dept_name);
        $this->data['action'] = 'transactions';
        $this->data['start_date'] = $from = date('Y-m-d H:i:s', strtotime($this->input->post('start_date')));
        $this->data['end_date'] = $to = date('Y-m-d H:i:s', strtotime($this->input->post('end_date')));
...
public function posts()
{
        ...
        $group_id = $this->_check_dept($member)['group'];
        $can_update = $this->db->get_where('contacts', ['user_id' => $member])->row()->can_update;
  
        $totalData = $this->member_model->allposts_count($client->client_id,$dept,$sub_dept,$group_id);
            
        $totalFiltered = $totalData; 
            
        if(empty($this->input->post('search')['value']))
        {            
            $posts = $this->member_model->allposts($limit,$start,$order,$dir,$client->client_id,$dept,$sub_dept,$group_id);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->member_model->posts_search($limit,$start,$search,$order,$dir,$client->client_id,$dept,$sub_dept,$group_id);

            $totalFiltered = $this->member_model->posts_search_count($search,$client->client_id,$dept,$sub_dept,$group_id);
        }
...
public function transaction_server_side()
{
        ...
        $totalData = $this->member_model->t_allposts_count($org, $dept, $sub_dept, $group_id,$from,$to);
            
        $totalFiltered = $totalData; 
            
        if(empty($this->input->post('search')['value']))
        {            
            $posts = $this->member_model->t_allposts($limit,$start,$order,$dir,$org,$dept,$sub_dept,$group_id,$from,$to);
            $totalSum = $this->member_model->t_allposts_sum($org, $dept, $sub_dept, $group_id,$from,$to);
        }
        else {
            $search = $this->input->post('search')['value']; 

            $posts =  $this->member_model->t_posts_search($limit,$start,$search,$order,$dir,$org,$dept,$sub_dept,$group_id,$from,$to);
            $totalSum = $this->member_model->t_searchedposts_sum($search,$org, $dept, $sub_dept, $group_id,$from,$to);

            $totalFiltered = $this->member_model->t_posts_search_count($search,$org,$dept,$sub_dept,$group_id,$from,$to);
        }
        ...
        $json_data = array(
                    ...
                    "from"              => $from,
                    "to"              =>$to,
                    "sum"              =>$totalSum ? $totalSum->toll_sum : 0
                    );
        echo json_encode($json_data); 
}
