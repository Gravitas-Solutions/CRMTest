...
/*Vehicles*/
	...
	public function vehicle_by_id($id)
	{
		...
		$client_id = $this->db->select('client_id')->from('vehicles')->where('vehicle_id', $id)->get()->row()->client_id;
		($this->db->get_where('clients', ['client_id' => $client_id])->row()->sub_dept_exist) ? $sub_dept_exist = 1 : $sub_dept_exist = 0;
		$this->db->from('vehicles');
		$this->db->join('clients', 'clients.client_id = vehicles.client_id');
		$this->db->join('departments', 'departments.dept_id = vehicles.dept_id');
		if ($sub_dept_exist != 0) {
...
/*Invoices*/
	...
	public function transactions($org, $dept, $sub_dept, $group_id)
	{
		...
		if ($org == 'amazon' || $org == 'fleet_serv_pro') {
			$this->db->select('exit_name, '.$org.'_invoice.license_plate, tolltag, invoice_id, state_code, agency_name, exit_date_time, date_for, toll, unit'.(($org == 'clay_cooley_dealerships') ? ', processed' : ''));
		} else {
			$this->db->select($org.'_invoice.license_plate, state_code, invoice_id, agency_name, vin_no, tolltag, exit_date_time, date_for, toll, exit_lane, exit_location, unit'.(($org == 'clay_cooley_dealerships') ? ', processed' : ''));
		}
	...
	{
		if ($org == 'amazon' || $org == 'fleet_serv_pro') {
			$this->db->select('invoice_id, exit_name, '.$org.'_invoice.license_plate, tolltag, state_code, agency_name, exit_date_time, date_for, toll, unit');
		} else {
			$this->db->select($org.'_invoice.license_plate, invoice_id, state_code, agency_name, tolltag, vin_no, exit_date_time, date_for, toll, exit_lane, exit_location, unit'.(($org == 'clay_cooley_dealerships') ? ', processed' : ''));
		}
		$this->db->from($org.'_invoice');
		$this->db->join('vehicles', 'vehicles.license_plate = '.$org.'_invoice.license_plate');
    ...
    {   
		...
		if ($org == 'amazon' || $org == 'fleet_serv_pro') {
			$this->db->select('exit_name, '.$org.'_invoice.license_plate,invoice_id, state_code, agency_name, exit_date_time, toll');
		} else {
			$this->db->select($org.'_invoice.license_plate, state_code, invoice_id, agency_name, exit_date_time, toll, exit_lane, exit_location');
		}
    ...
    {   
		...
		if ($org == 'amazon' || $org == 'fleet_serv_pro') {
			$this->db->select('exit_name, '.$org.'_invoice.license_plate, invoice_id, state_code, agency_name, exit_date_time, toll, dept_name');
		} else {
			$this->db->select($org.'_invoice.license_plate, state_code, invoice_id, agency_name, exit_date_time, toll, exit_lane, exit_location, dept_name');
		}
